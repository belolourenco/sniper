...
void Why3Gen::init(){
  W3UsePtr useInt = W3Decl::mkUseImport("int.Int");
  W3UsePtr useBool = W3Decl::mkUseImport("bool.Bool");
  W3UsePtr useDiv = W3Decl::mkUseImport("int.ComputerDivision");
  W3UsePtr useArray = W3Decl::mkUseImport("array.Array");
...
void Why3Gen::addToTheory(ExprPtr e){
  W3ExprPtr w3e = encodeAsGoal(e);
  W3GoalPtr goal = W3Decl::mkGoal(std::to_string(vcCounter++), w3e);

  theory.add(goal);
}

void Why3Gen::addToTheory(const std::string &name, ExprPtr e){
  W3ExprPtr w3e = encodeAsGoal(e);
  W3GoalPtr goal = W3Decl::mkGoal(name, w3e);

  theory.add(goal);
}

void Why3Gen::addToTheory(const std::vector<ExprPtr> &es){
  for (ExprPtr e : es){
    W3ExprPtr w3e = encodeAsGoal(e);
    W3GoalPtr goal = W3Decl::mkGoal(std::to_string(vcCounter++),w3e);
    theory.add(goal); 
...
const W3Theory &Why3Gen::getTheory(){
  return theory;
}

W3ExprPtr Why3Gen::encodeAsGoal(ExprPtr e){
  W3ExprPtr ee = makeWhy3Expr(e);
  return W3Expr::mkUQuant(int_vars,bool_vars,int_to_int_vars,ee);
}

W3ExprPtr Why3Gen::makeWhy3Expr(ExprPtr e){
  switch (e->getOpCode()) {
    ...
    case Expression::IntVar: {
        ...
        if(ie -> getName() == ""){
            ...
            s = (ie -> getName() == "div")?"_div_":ie -> getName();
            replace(s.begin(), s.end(), '.', '_' );
        }
        p.int_vars++;
...
void Why3Gen::prettyprintTheory(raw_ostream &os){
  theory.prettyprint(os);
}
